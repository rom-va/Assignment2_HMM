...
class Localizer:
    ...
    def update(self) -> (bool, int, int, int, int, int, int, int, int, np.array(1)) :
        self.__trueState = self.__rs.move_once(self.__tm)                           # Robot sim moves once and stores currentState
        self.__sense = self.__rs.sense_in_current_state(self.__om)                  # Robot sim senses and return state index
        self.__probs = self.__HMM.filter( self.__sense)                             # The sensed state is passed to the filter
